...
Project_name = 'Excel Batch Compare';
Current_dir = os.getcwd() + '\\';
AssemblyInfo_path = Current_dir + r'comp-lib\Properties\AssemblyInfo.cs';
VersionDll_path = Current_dir + r'comp-lib\bin\Release\ExcelCompare.exe';
iss_path = Current_dir + r'comp-package.iss';
...
ReplaceInFile( Current_dir + r'comp-exe\Properties\AssemblyInfo.cs', r'Version\("[.\d]+"\)', r'Version("' + NewVersion + '")' )
ReplaceInFile( Current_dir + r'comp-lib\Properties\AssemblyInfo.cs', r'Version\("[.\d]+"\)', r'Version("' + NewVersion + '")' )
ReplaceInFile( Current_dir + r'comp-runner\Properties\AssemblyInfo.cs', r'Version\("[.\d]+"\)', r'Version("' + NewVersion + '")' )

print( "** Clear previous compilations ...")
DeleteFolder(Current_dir + r'comp-exe\bin' );
DeleteFolder(Current_dir + r'comp-exe\obj' );
DeleteFolder(Current_dir + r'comp-lib\bin' );
DeleteFolder(Current_dir + r'comp-lib\obj' );
DeleteFolder(Current_dir + r'comp-runner\bin' );
DeleteFolder(Current_dir + r'comp-runner\obj' );

print( "** Compile main library ...")
if( not MsBuild( Current_dir + r'comp-lib\comp-lib.csproj' ) ): exit(1)

print( "** Compile application ...")
if( not MsBuild( Current_dir + r'comp-exe\comp-exe.csproj' ) ): exit(1)

print( "** Compile runner ...")
if( not MsBuild( Current_dir + r'comp-runner\comp-runner.csproj' ) ): exit(1)
